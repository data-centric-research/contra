...
class PLF(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.model_state, self.optimizer_state, self.model_ema, self.model_anchor = (
            copy_model_and_optimizer(self.model, self.optimizer, self.custom_args)
        )
        self.transform = get_tta_transforms(dataset=self.custom_args.dataset)
        self.mt = mt_alpha
        self.rst = rst_m
        self.ap = ap
        self.ap_pre = 0
    ...
    def update(self, probs_x_ulb):
        ...
        hist = torch.bincount(max_idx.view(-1), minlength=self.p_model.shape[0]).to(
            self.p_model.dtype
        )
    ...
    def masking(self, logits_x_ulb, softmax_x_ulb=True, *args, **kwargs):
        ...
        probs_x_ulb = (
            torch.softmax(logits_x_ulb.detach(), dim=-1)
            if softmax_x_ulb
    ...
    def forward_and_adapt(self, x, model, optimizer):
        ...
        ap_alter = (
            torch.nn.functional.softmax(outputs_ema + outputs_test, dim=1)
            .max(1)[0]
            .mean(0)
            - self.ap_pre
        )
        ...
        if ap_alter < 0:
            self.time_p = exponential_decay_threshold(
                self.time_p, ap_alter, decay_factor
            )
        ...
        loss2 = (
            0.5 * symmetric_cross_entropy(outputs_test, outputs_ema * mask1)
            + 0.5 * symmetric_cross_entropy(outputs_aug_test, outputs_ema * mask1)
        ).mean(0)
...
def copy_model_and_optimizer(model, optimizer, args):
    ...
    # dora modify ema_model from p0
    ema_model = deepcopy(model_p0)
